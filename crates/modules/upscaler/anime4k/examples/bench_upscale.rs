...
use anime4k::{Anime4KModel, Anime4KUpscaler};
...
/// Usage (from repo root): bench_upscale [all|cpu] [2x_S|2x_M|3x_L|3x_VL|4x_UL] [image path] [longest side]
///
...
async fn main() {
    ...
    let variant = args.next().unwrap_or_else(|| "2x_S".into());
    let img_path = args
        .next()
    ...
    let model = match variant.as_str() {
        "2x_S" => Anime4KModel::X2S,
        "2x_M" => Anime4KModel::X2M,
        "3x_L" => Anime4KModel::X3L,
        "3x_VL" => Anime4KModel::X3VL,
        "4x_UL" => Anime4KModel::X4UL,
        _ => panic!("unknown variant {variant}"),
    };
    let up = Anime4KUpscaler::new(model, providers);

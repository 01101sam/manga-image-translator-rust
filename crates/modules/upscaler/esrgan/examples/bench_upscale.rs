...
/// Usage (from repo root): bench_upscale [all|cpu] [x2plus|x4plus|x4plus_anime_6B]-[f16|f32] [image path] [longest side]
///
...
async fn main() {
    ...
    let variant = args.next().unwrap_or_else(|| "x4plus-f16".into());
    let img_path = args
        .next()
    ...
    let f32 = variant.ends_with("-f32");
    let model = match variant.rsplit_once('-').map(|v| v.0) {
        Some("x2plus") => EsrGanModel::X2Plus { f32 },
        Some("x4plus") => EsrGanModel::X4Plus { f32 },
        Some("x4plus_anime_6B") => EsrGanModel::X4PlusAnime6B { f32 },
        _ => panic!("unknown variant {variant}"),
    };
    let up = EsrGan::new(model, 5, providers);
